...
const graph_data = { '0': { n: 10, s: 2, e: 4, w: 1 },
'1': { e: 0 },
'2': { n: 0, s: 6, e: 3 },
'3': { s: 9, e: 5, w: 2 },
'4': { n: 23, e: 13, w: 0 },
...
let data_file = fs.readFileSync("./data.json", { encoding: "utf-8" })

data_file = data_file.substring(0, data_file.length - 1)

const lines = []

let string = data_file[0]
for(let i = 1, n = data_file.length; i < n; i++) {
    const prevChar = data_file[i - 1]
    const char = data_file[i]
...
const all_lines = JSON.parse("[" + data_file + "]")

const compiled = {}

for(let object of all_lines) {
    const id = object.room_id

    object['directions'] = graph_data[id]

    compiled[id] = object

    delete compiled['players']
}
